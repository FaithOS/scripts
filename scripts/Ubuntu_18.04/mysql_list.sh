#!/bin/bash
# mysql服务 mysql_list.sh
#

#mysqlDIR 这个变量定义在myprofile.sh文件
###新的菜单只需要替换 ${mysqlDIR} 和mysql 相关的文件即可使用 
###二级菜单的环境变量 ${mysqlDIR} 在文件check_OS.sh内定义  
#set -e 
##定义菜单函数
#CLEAN(){
...
SELECT_lista(){
  ...
  for A in $(ls -al ${mysql_DIR} | grep "^-"  |awk '{print $9}')          #查看目录下所有文件名并且赋值A，为当前系统下的所有脚本>名称
    do
      sed -n '2p' ${mysql_DIR}/"$A" >>${TMPDIR}/mysql_file.txt               #将所有脚本的第二行内容打印到test/new.txt 文件
  done 
}
...
#####################
     DESCRIBES=$(awk  '{print $2}' ${TMPDIR}/mysql_file.txt)   #将文件内的第三列为 脚本描述 全部定义到ZD 这个变量内，用于后面对比
...
do
	...
	do
	    ...
	    implement_scripts=`grep "${array[$i]}"  ${TMPDIR}/mysql_file.txt |awk -F "${array[$i]}" '{print $2}'`  #如果对比成功就讲这个值得第二列取出，
	    cd ${mysql_DIR}
